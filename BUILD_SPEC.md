...
1. **Keys:** safeStorage -> gnome-libsecret; electron-store for non-secret config only; refuse to persist secrets on the `basic_text` backend.
...
8. Packaging: PyInstaller onedir sidecars, AppImage + .deb, first-run resumable model download with verification whenever authoritative metadata is available.
